...
from NLP_pretraining.bert import BERTModel, get_tokens_and_segments
from datasets.NLinference_dataset import read_snli
from utils.TextPreprocessing import Vocab, tokenize
from utils.train_ch13 import train_ch13


def load_pretrained_model(pretrained_model, num_hiddens, ffn_num_hiddens,
                          num_heads=4, num_layers=2, dropout=0.2, max_len=512):
...
class SNLIBERTDataset(Dataset):
    def __init__(self, dataset, max_len, vocab=None):
        tokens_pair = [tokenize([s.lower() for s in sentences])
                       for sentences in dataset[:2]]
        all_premise_hypothesis_tokens = [[p_tokens, h_tokens]
                                         for p_tokens, h_tokens in zip(*tokens_pair)]
...
if __name__ == "__main__":
    ...
    # test SNLIBERTDataset
    # 如果出现显存不足错误，请减少“batch_size”。在原始的BERT模型中，max_len=512
    batch_size, max_len, num_workers = 512, 128, 0   #d2l.get_dataloader_workers()
    ...
    params_1x = [param for name, param in net.named_parameters()
                 if name not in ('hidden.0.weight', 'hidden.0.bias',
                                'output.weight', 'output.bias')]
    params_10x = [param for name, param in net.named_parameters()
                  if name in ('hidden.0.weight', 'hidden.0.bias',
                                  'output.weight', 'output.bias')]
