...
from NLP_pretraining.bert import BERTModel
from datasets.bert_dataset import load_data_wiki
    ...
    while step < num_steps and not num_steps_reached:
        for data in train_iter:
            tokens_X, segments_X, valid_lens_x, pred_positions_X, \
                mlm_weights_X, mlm_Y, nsp_y = to_device(data, device)
            trainer.zero_grad()
            timer.start()
...
def get_bert_encoding(net, tokens_a, tokens_b=None, device=d2l.try_gpu()):
